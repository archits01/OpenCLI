...
var activeBindings = /* @__PURE__ */ new Map();
var _pushEventCallback = null;
function setPushEventCallback(cb) {
	_pushEventCallback = cb;
}
var NETWORK_PUSH_BINDING = "__oc_netcapture_push";
var networkPushTabs = /* @__PURE__ */ new Set();
var NETWORK_PUSH_URL_RE = /deliveryAck|deliveryAcknowledgement|messaging|realtime|messenger|mercury|voyagerMessaging/i;
function enableNetworkPush(tabId) {
	networkPushTabs.add(tabId);
}
function disableNetworkPush(tabId) {
	networkPushTabs.delete(tabId);
}
function maybePushNetworkEntry(tabId, entry) {
	if (!networkPushTabs.has(tabId) || !_pushEventCallback) return;
	const url = entry.url || "";
	if (!(entry.method || "").startsWith("WS_") && !NETWORK_PUSH_URL_RE.test(url)) return;
	try {
		_pushEventCallback(tabId, NETWORK_PUSH_BINDING, JSON.stringify(entry));
	} catch {}
}
var streamArmedRequests = /* @__PURE__ */ new Set();
var streamConsumedLen = /* @__PURE__ */ new Map();
var wsUrlByRequestId = /* @__PURE__ */ new Map();
var STREAM_CAPTURE_URL_RE = /realtime|voyagerMessagingGraphQL|messengerConversations|messengerMessages|mercury|subscribe/i;
...
var workerTargetToTab = /* @__PURE__ */ new Map();
...
async function attachWorkerTargets(tabId) {
	try {
		...
		for (const target of all) {
			...
			if ((ttype === "worker" || ttype === "shared_worker" || ttype === "service_worker" || ttype === "other") && /linkedin\.com/i.test(url) && target.id && !target.attached) await attachSingleWorkerTarget(tabId, target.id);
		}
	} catch {}
}
async function attachSingleWorkerTarget(tabId, targetId) {
	if (workerTargetToTab.has(targetId)) return;
	try {
		await chrome.debugger.attach({ targetId }, "1.3");
	} catch (err) {
		if (!(err instanceof Error ? err.message : String(err)).includes("Another debugger is already attached")) return;
	}
	workerTargetToTab.set(targetId, tabId);
	try {
		await chrome.debugger.sendCommand({ targetId }, "Network.enable");
	} catch {}
}
/**
...
*/
async function enumerateTargets(tabId) {
	try {
		return (await new Promise((resolve) => {
			chrome.debugger.getTargets((targets) => resolve(targets || []));
		})).map((t) => ({
...
*/
async function forceRealtimeReconnect(tabId) {
	await ensureAttached(tabId);
	await attachWorkerTargets(tabId);
	const debuggees = [{ tabId }];
	for (const [targetId, owner] of workerTargetToTab.entries()) if (owner === tabId) debuggees.push({ targetId });
	const offline = {
		offline: true,
		latency: 0,
	...
	try {
		for (const d of debuggees) try {
			await chrome.debugger.sendCommand(d, "Network.emulateNetworkConditions", offline);
		} catch {}
...
async function restoreOnline(debuggees) {
	const online = {
		offline: false,
		latency: 0,
		downloadThroughput: -1,
	...
	for (const d of debuggees) for (let attempt = 0; attempt < 2; attempt++) try {
...
*/
async function ensureOnline(tabId) {
	try {
		await ensureAttached(tabId);
	} catch {
		return;
...
function clearWorkerTargetsForTab(tabId) {
...
function registerListeners() {
	...
	chrome.debugger.onEvent.addListener(async (source, method, params) => {
		...
		if (!tabId && source.targetId) tabId = workerTargetToTab.get(source.targetId);
		if (!tabId) return;
		const state = networkCaptures.get(tabId);
		if (!state) return;
		const eventParams = params;
		const debuggee = source.targetId && workerTargetToTab.has(source.targetId) ? { targetId: source.targetId } : { tabId };
		if (method === "Network.requestWillBeSent") {
			...
			try {
				const postData = await chrome.debugger.sendCommand(debuggee, "Network.getRequestPostData", { requestId });
		...
		if (method === "Network.responseReceived") {
			...
			if (networkPushTabs.has(tabId) && STREAM_CAPTURE_URL_RE.test(entry.url || "") && !streamArmedRequests.has(requestId)) {
				if (streamArmedRequests.size > 500) {
					streamArmedRequests.clear();
					streamConsumedLen.clear();
				}
				streamArmedRequests.add(requestId);
				try {
					const buffered = await chrome.debugger.sendCommand(debuggee, "Network.streamResourceContent", { requestId });
		...
		if (method === "Network.loadingFinished") {
			...
			if (!entry) return;
			try {
				const body = await chrome.debugger.sendCommand(debuggee, "Network.getResponseBody", { requestId });
		...
		if (method === "Network.eventSourceMessageReceived") {
			...
			const url = (parentIndex !== void 0 ? state.entries[parentIndex] : void 0)?.url || "";
		...
		if (method === "Network.dataReceived") {
			...
			if (networkPushTabs.has(tabId) && STREAM_CAPTURE_URL_RE.test(url)) {
				try {
					const sc = await chrome.debugger.sendCommand(debuggee, "Network.streamResourceContent", { requestId });
					if (sc?.bufferedData) {
						streamArmedRequests.add(requestId);
			...
			const now = Date.now();
			if (now - (parentEntry._lastDataReceived || 0) < 1e3) return;
			parentEntry._lastDataReceived = now;
			try {
				const body = await chrome.debugger.sendCommand(debuggee, "Network.getResponseBody", { requestId });
				if (body?.body) {
					const fullSize = body.body.length;
		...
		if (method === "Runtime.bindingCalled") {
			...
			if (name && _pushEventCallback) {
				if (activeBindings.get(tabId)?.has(name)) _pushEventCallback(tabId, name, payload);
			}
			return;
		}
	});
}
async function addBinding(tabId, name) {
	await ensureAttached(tabId);
...
async function removeBinding(tabId, name) {
...
var LEGACY_AUTOMATION_TAB_GROUP_TITLE = "OpenCLI";
...
function initialize() {
	...
	registerListeners();
	try {
		setPushEventCallback((tabId, name, payload) => {
			safeSend(ws, {
				type: "push-event",
				tabId,
				name,
...
async function handleCommand(cmd) {
	...
	try {
		switch (cmd.action) {
			...
			case "add-binding": return await handleAddBinding(cmd, leaseKey);
			case "remove-binding": return await handleRemoveBinding(cmd, leaseKey);
			default: return {
				id: cmd.id,
				ok: false,
...
async function handleAddBinding(cmd, leaseKey) {
	if (!cmd.bindingName) return {
		id: cmd.id,
		ok: false,
		error: "Missing bindingName"
	};
	const tabId = await resolveTabId(await resolveCommandTabId(cmd), leaseKey);
	try {
		if (cmd.bindingName === "__oc_netcapture_push") enableNetworkPush(tabId);
		else if (cmd.bindingName === "__oc_recon_targets") {
			const targets = await enumerateTargets(tabId);
			return pageScopedResult(cmd.id, tabId, { targets });
		} else if (cmd.bindingName === "__oc_force_reconnect") {
			const r = await forceRealtimeReconnect(tabId);
			return pageScopedResult(cmd.id, tabId, r);
		} else if (cmd.bindingName === "__oc_ensure_online") {
			await ensureOnline(tabId);
			return pageScopedResult(cmd.id, tabId, { online: true });
		} else await addBinding(tabId, cmd.bindingName);
		return pageScopedResult(cmd.id, tabId, {
			bound: true,
			name: cmd.bindingName
		});
	} catch (err) {
		return {
			id: cmd.id,
...
async function handleRemoveBinding(cmd, leaseKey) {
	if (!cmd.bindingName) return {
		id: cmd.id,
		ok: false,
		error: "Missing bindingName"
	...
	const tabId = await resolveTabId(await resolveCommandTabId(cmd), leaseKey);
	try {
		if (cmd.bindingName === "__oc_netcapture_push") disableNetworkPush(tabId);
		else await removeBinding(tabId, cmd.bindingName);
		return pageScopedResult(cmd.id, tabId, {
			removed: true,
			name: cmd.bindingName
		});
